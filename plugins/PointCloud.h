...
class PointCloudPlugin: public pubviz::Plugin
{
	...
	std::unique_ptr<StringProperty> point_text_;
...
//will need matrix classes and invert
	...
	virtual std::map<std::string, std::string> Select(uint32_t index, pubviz::AABB& size) override
	{
		...
		for (auto& cloud: clouds_)
		{
			...
			if (index >= current && index < end_index)
			{
				...
				float y = fdata[i*increment+1];
				float z = fdata[i*increment+2];
	...
	void StringConfigChanged(const std::string& str)
	{
		// Recolor all clouds
		for (auto& cloud: clouds_)
		{
